...
def check_complete_records(bucket: str, prefix: str) -> int:
    ...
    Args:
        ...
        prefix: prefijo de la clave S3 con el contador.

    Returns:
        Número de registros completos desde el último entrenamiento.
    """
    try:
        import boto3
...
def trigger_pipeline(pipeline_name: str) -> str:
    ...
    Args:
        pipeline_name: nombre del pipeline en SageMaker.

    Returns:
        ARN de la ejecución iniciada (o string descriptivo en modo local).
    """
    try:
        import boto3
...
def handler(event: dict, context) -> dict:
    ...
    Args:
        event: payload de la Lambda (puede contener override de threshold).
        context: contexto de Lambda (puede ser None en modo local).

    Returns:
        dict con statusCode y body describiendo la decisión.
    """
    ...
    # Allow explicit override in event for local simulation / testing
    ...
    logger.info(
        "Registros completos nuevos: %d (threshold: %d)", new_records, threshold
    )
...
if __name__ == "__main__":
    ...
    result = handler({"new_complete_records": 55}, None)
